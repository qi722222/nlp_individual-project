...
# transformers 5.x removed the conversion script, so fetch it from the 4.44.0 tag.
...
echo "[convert] downloading conversion script ..."
...
echo "[convert] script downloaded: $(wc -l < ${SCRIPT_PATH}) lines"
echo "[convert] input:  ${INPUT_DIR}"
echo "[convert] output: ${OUTPUT_DIR}"
